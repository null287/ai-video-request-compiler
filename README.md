...
不需要下载仓库，也不需要了解其中的文件结构。仓库更新后，继续使用同一个链接即可。

如果当前对话无法访问网页，请直接上传 [`llms-full.txt`](llms-full.txt)，再发送你的需求。

## 它会做什么

AI 读取仓库后，会把你提供的内容作为原始需求：

1. 区分已确认信息、待确认项和风险/冲突；
2. 不擅自补写未经确认的剧情、人物关系、产品功能或台词；
3. 信息不足时先提出必要问题；
4. 信息充分时输出结构化的《AI 视频执行稿》；
5. 在生成前检查时长、素材、镜头、声音、成本和验收条件。

只发送仓库链接而没有具体需求时，AI 应加载规范并询问需要处理的需求，不应凭空生成项目内容。

## 给 AI：从这里开始

如果用户要求你按照本仓库处理需求，请执行以下流程，而不是只总结仓库：
